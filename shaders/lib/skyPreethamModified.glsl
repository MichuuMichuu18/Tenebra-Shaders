...
vec3 sunDisc(vec3 viewDir, vec3 sunDir) {
	...
	vec3 haloColor = vec3(1.0, 0.95, 0.5); // for some reason it might be a bit too pink
...
vec3 calcFogColor(vec3 viewDir, vec3 sunDir) {
    ...
    fog = mix(fog, vec3(dot(fog, vec3(0.299, 0.587, 0.114))), rainStrength * 0.6);
...
vec3 calcSkyColorPreetham(vec3 viewDir) {
	...
	// Base colors
	vec3 zenithColor  = skyColor * 0.75 + rainStrength * 0.25;
	...
	vec3 nightHorizon = vec3(0.052, 0.091, 0.117);
